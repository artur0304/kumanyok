...
export type SiteSettings={open:number;close:number;saunaExtra:number;extraHourly:boolean;hoursConfirmed:boolean;paymentCard:string;minDeposit:number};
const gazeboPhotos=[
...
export const initialResources:Resource[] = [[1600,1800],[1600,1800],[1500,1700],[1600,1800],[2900,3400],[2000,2300],[2400,2700],[2400,2600],[1600,1600],[2400,2900]].map((p,i)=>{const id=`gazebo-${i+1}`,fallback=gazeboPhotos[i],photo=`/images/ai/${id}.webp`;return {id,name:`Альтанка №${i+1}`,weekday:p[0],weekend:p[1],weekendStart:i===9?5:6,kind:'gazebo',photo,photos:[photo,...photosFor(id,fallback)],description:gazeboDescriptions[i]}});
...
export const defaults:SiteSettings={open:9,close:22,saunaExtra:100,extraHourly:true,hoursConfirmed:false,paymentCard:'4149499156747394',minDeposit:500};
