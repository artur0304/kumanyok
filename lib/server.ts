...
export async function catalog(){const row=await db().prepare('SELECT value FROM settings WHERE id = ?').bind('catalog').first<{value:string}>();if(!row)return {resources:initialResources,settings:defaults};const saved=JSON.parse(row.value) as {resources:Resource[];settings:typeof defaults};return {...saved,resources:initialResources.map(base=>({...base,...saved.resources.find(r=>r.id===base.id),photo:base.photo,photos:base.photos,description:base.description}))};}
