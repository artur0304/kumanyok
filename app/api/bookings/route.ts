...
const booking=z.object({
 ...
 guests:z.number().int().min(1).max(100),deposit:z.number().int().min(1),
...
export async function POST(req:Request){
 try{
  ...
  if(b.deposit<settings.minDeposit||b.deposit>total)return Response.json({error:`Передоплата має бути від ${settings.minDeposit} грн до повної суми.`},{status:400});
  ...
  const result=await db().prepare("INSERT INTO bookings (id,resource,date,start,end,guests,name,phone,total,deposit,status,source,note,created,requested_start,requested_end) SELECT ?,?,?,?,?,?,?,?,?,?,'pending','site','',?,?,? WHERE NOT EXISTS (SELECT 1 FROM bookings WHERE resource=? AND date=? AND status='confirmed' AND start<? AND end>?) AND NOT EXISTS (SELECT 1 FROM bookings WHERE resource=? AND date=? AND phone=? AND status='pending' AND start<? AND end>?)")
   .bind(id,r.id,b.date,start,end,b.guests,b.name,b.phone,total,b.deposit,now.toISOString(),b.start,b.end,r.id,b.date,end,start,r.id,b.date,b.phone,end,start).run();
  if(result.meta.changes!==1)return Response.json({error:'Цей час уже підтверджено або така заявка вже надіслана.'},{status:409});
